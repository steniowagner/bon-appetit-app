...
const verifyResourceAlreadyExists = async (path: string): string => {
  const resourceAlreadyExists = await RNFetchBlob.fs.exists(path);

  return resourceAlreadyExists;
};

const getItemFromCache = async (uri: string, folder: string = 'images', fileExtension: string = 'jpg'): string => {
  ...
  const isDirectoryAlreadyCreated = await verifyResourceAlreadyExists(fileDirectory);
